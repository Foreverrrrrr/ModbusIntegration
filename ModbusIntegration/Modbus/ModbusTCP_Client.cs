...
namespace ModbusIntegration.Modbus
{
    public class AsyncSharpTcpClient
    {
        ...
        public void OnInteractionEvent(TimeSpan timeSpan, bool result, string readWrite, string startAddress, int registerCount, string operationValue)
        {
            ...
            InteractionEvent?.BeginInvoke(timeSpan, result, readWrite, startAddress, registerCount, operationValue, null, null);
            GC_Lock++;
        }
                ...
                if (IsConnect)
                {
                    ...
                    if (bytesRead > 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                syncSend.DataBuff[i] = syncSend.ReceiveBuff[8 + i];
                            }
        ...
        public Package<int> WriteIn32(string address, int value)
        {
            ...
            if (IsConnect)
            {
                ...
                accept.SendBuff = GetWriteMessage(0x10, address, bytes);
                SyncSendReceive(ref accept);
                accept.DataBuff = new byte[5];
                Array.Copy(accept.ReceiveBuff, accept.ReceiveBuff.Length - 5, accept.DataBuff, 0, accept.DataBuff.Length);
            }
